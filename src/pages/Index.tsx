  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="mx-4 mt-4 md:mx-8">
        <BannerSlider banners={heroSlides} />
      </section>

      {/* Small Strip Banner – Free Shipping */}
      <section className="mx-4 md:mx-8 mt-4">
        <PromoBanner title="Free Shipping" subtitle="on orders above ₹499" variant="small" link="/category/skincare" />
      </section>
      ...
      <section className="mx-4 md:mx-8 mt-6">
        <PromoBanner title="Glowing Skin Essentials" subtitle="all under ₹499" variant="small" link="/category/skincare" bgColor="bg-primary" />
      </section>
      ...
      <section className="mx-4 md:mx-8 mt-8">
        <PromoBanner image={newArrivals} title="New Arrivals" subtitle="Discover the latest beauty trends" cta="Shop Now" variant="full" link="/category/new-arrivals" />
      </section>

      {/* Vertical Banners + Products Mix */}
      <section className="container mx-auto px-4 mt-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-[1fr_2fr_1fr] gap-4 mt-4">
          <PromoBanner image={fragrances} title="Exclusive Fragrances" subtitle="Starting ₹299" variant="vertical" link="/category/fragrance" className="hidden md:block" />
          <div className="grid grid-cols-2 gap-4 col-span-2 md:col-span-1">
            {trendingDeals.length > 0 ? trendingDeals.slice(0, 4).map(p => <ProductCard key={p.id} product={p} />) : currentHits.slice(0, 4).map(p => <ProductCard key={p.id} product={p} />)}
          </div>
          <PromoBanner image={lipFest} title="Lip Fest" subtitle="All lip products on sale" variant="vertical" link="/category/lip-care" className="hidden md:block" />
        </div>
      </section>

      {/* Horizontal Banner – Glow Up */}
      <section className="mx-4 md:mx-8 mt-8">
        <PromoBanner image={glowUp} title="Glow Up Sale" subtitle="Skincare at unbeatable prices" cta="Explore" variant="full" link="/category/skincare" />
      </section>
      ...
      {/* Bottom Banner Strip */}
      <section className="mx-4 md:mx-8 mt-8">
        <PromoBanner image={freeShipping} title="Free Shipping on ₹499+" variant="horizontal" link="/category/bestsellers" />
      </section>
